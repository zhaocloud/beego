...
func TestCRUD(t *testing.T) {
	...
	err = dORM.Read(u)
	throwFailNow(t, err)
	throwFail(t, AssertIs(u.UserName, "zhaocloud"))
	throwFail(t, AssertIs(u.Profile.Id, profile.Id))

	u = &User{UserName: "zhaocloud", Password: "pass"}
...
func TestInsertTestData(t *testing.T) {
	...
	user = NewUser()
	user.UserName = "zhaocloud"
	user.Email = "zhaocloud@gmail.com"
...
func TestOrderBy(t *testing.T) {
	...
	num, err = qs.OrderBy("-profile__age").Filter("user_name", "zhaocloud").Count()
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
}
...
func TestAll(t *testing.T) {
	...
	throwFail(t, AssertIs(users[1].UserName, "zhaocloud"))
	...
	throwFailNow(t, AssertIs(users2[1].UserName, "zhaocloud"))
...
func TestValuesFlat(t *testing.T) {
	...
	if num == 3 {
		...
		throwFail(t, AssertIs(list[1], "zhaocloud"))
...
func TestRelatedSel(t *testing.T) {
	...
	throwFailNow(t, AssertIs(posts[1].User.UserName, "zhaocloud"))
	throwFailNow(t, AssertIs(posts[2].User.UserName, "zhaocloud"))
...
func TestReverseQuery(t *testing.T) {
	...
	profile = Profile{}
	err = dORM.QueryTable("user_profile").Filter("User__UserName", "zhaocloud").One(&profile)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "zhaocloud"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "zhaocloud").Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "zhaocloud"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "zhaocloud").RelatedSel().Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "zhaocloud"))
	...
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "zhaocloud").All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	tags = []*Tag{}
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "zhaocloud").RelatedSel().All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	throwFailNow(t, AssertIs(tags[0].BestPost.User.UserName, "zhaocloud"))
}
...
func TestLoadRelated(t *testing.T) {
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(len(user.Posts), 2))
	throwFailNow(t, AssertIs(user.Posts[0].User.UserName, "zhaocloud"))
	...
	throwFailNow(t, AssertIs(profile.User.UserName, "zhaocloud"))
	...
	throwFailNow(t, AssertIs(post.User.UserName, "zhaocloud"))
	...
	throwFailNow(t, AssertIs(post.Tags[0].BestPost.User.UserName, "zhaocloud"))
...
func TestQueryRows(t *testing.T) {
	...
	throwFailNow(t, AssertIs(usernames[1], "zhaocloud"))
